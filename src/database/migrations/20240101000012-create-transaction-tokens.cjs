...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    await queryInterface.addIndex('transaction_tokens', ['token'], {
      name: 'idx_transaction_tokens_token',
      unique: true,
    });

    // Index on transaction_id for finding tokens by transaction
    await queryInterface.addIndex('transaction_tokens', ['transaction_id'], {
      name: 'idx_transaction_tokens_transaction_id',
    });

    // Index on expires_at for cleanup queries
    await queryInterface.addIndex('transaction_tokens', ['expires_at'], {
      name: 'idx_transaction_tokens_expires_at',
    });
